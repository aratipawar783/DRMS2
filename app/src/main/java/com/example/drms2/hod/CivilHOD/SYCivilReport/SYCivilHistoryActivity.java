...
public class SYCivilHistoryActivity extends AppCompatActivity {
    ...
    public void deleteReport() {
        String date = etDate.getText().toString().trim();
        String roll=etRollNo.getText().toString().trim();// Get date from EditText

        if (date.isEmpty()|| roll.isEmpty()) {
            Toast.makeText(this, "Please enter a date and roll number", Toast.LENGTH_SHORT).show();
            return;
        }
        if(roll.startsWith("CE2")&&(! date.isEmpty()))
        {
        // Call the delete method from DatabaseHelper
        int deletedRows = dbHelper.deleteReportsByDate(date,roll); // Delete by date
