...
public class CropViewTYAE extends View {
    ...
    public CropViewTYAE(Context context) {
        super(context);
        init();
    }

    public CropViewTYAE(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    public void cropImage() {
        if (bitmap != null && cropRect.width() > 0 && cropRect.height() > 0) {
            ...
            if (getContext() instanceof MiddleActivityTYAE) {
                ((MiddleActivityTYAE) getContext()).setCroppedImage(croppedBitmap); // Pass cropped bitmap to MainActivity
            }
        }
    }
}
