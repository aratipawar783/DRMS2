...
public class MiddleActivityTYAE extends AppCompatActivity {

    private CropViewTYAE cropView;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        byte[] byteArray = getIntent().getByteArrayExtra("screenshottyae");
        if (byteArray != null) {
        ...
        sendButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (croppedImage != null) {
                    ...
                    Toast.makeText(MiddleActivityTYAE.this, "Image saved. You can navigate back to the dashboard.", Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(MiddleActivityTYAE.this, "Please crop the image first.", Toast.LENGTH_SHORT).show();
                }
            }
        });
    ...
    private void saveImageToFile(Bitmap bitmap) {
        File file = new File(getExternalFilesDir(null), "cropped_image_middletyae.png");
        try (FileOutputStream out = new FileOutputStream(file)) {
            bitmap.compress(Bitmap.CompressFormat.PNG, 100, out);
            out.flush();
            // Save the image URI to SharedPreferences
            SharedPreferences.Editor editor = sharedPreferences.edit();
            editor.putString("imageUrityaeForMiddle", file.getAbsolutePath());
            editor.apply();
        } catch (IOException e) {
            e.printStackTrace();
