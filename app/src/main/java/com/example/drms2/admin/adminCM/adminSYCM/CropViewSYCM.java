...
public class CropViewSYCM extends View {
    ...
    public CropViewSYCM(Context context) {
        super(context);
        init();
    }

    public CropViewSYCM(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    public void cropImage() {
        if (bitmap != null && cropRect.width() > 0 && cropRect.height() > 0) {
            ...
            if (getContext() instanceof MiddleActivitySYCM) {
                ((MiddleActivitySYCM) getContext()).setCroppedImage(croppedBitmap); // Pass cropped bitmap to MainActivity
            }
        }
    }
}
