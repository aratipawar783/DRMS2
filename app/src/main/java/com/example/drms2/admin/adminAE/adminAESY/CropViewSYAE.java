...
public class CropViewSYAE extends View {
    ...
    public CropViewSYAE(Context context) {
        super(context);
        init();
    }

    public CropViewSYAE(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    public void cropImage() {
        if (bitmap != null && cropRect.width() > 0 && cropRect.height() > 0) {
            ...
            if (getContext() instanceof MiddleActivitySYAE) {
                ((MiddleActivitySYAE) getContext()).setCroppedImage(croppedBitmap); // Pass cropped bitmap to MainActivity
            }
        }
    }
}
