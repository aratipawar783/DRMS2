...
public class CropViewFYCE extends View {
    ...
    public CropViewFYCE(Context context) {
        super(context);
        init();
    }

    public CropViewFYCE(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    public void cropImage() {
        if (bitmap != null && cropRect.width() > 0 && cropRect.height() > 0) {
            ...
            if (getContext() instanceof MiddleActivityFYCE) {
                ((MiddleActivityFYCE) getContext()).setCroppedImage(croppedBitmap); // Pass cropped bitmap to MainActivity
            }
        }
    }
}
