...
public class CropViewEJSY extends View {
    ...
    public CropViewEJSY(Context context) {
        super(context);
        init();
    }

    public CropViewEJSY(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    public void cropImage() {
        if (bitmap != null && cropRect.width() > 0 && cropRect.height() > 0) {
            ...
            if (getContext() instanceof MiddleActivityEJSY) {
                ((MiddleActivityEJSY) getContext()).setCroppedImage(croppedBitmap); // Pass cropped bitmap to MainActivity
            }
        }
    }
}
