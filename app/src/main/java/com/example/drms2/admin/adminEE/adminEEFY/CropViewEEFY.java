...
public class CropViewEEFY extends View {
    ...
    public CropViewEEFY(Context context) {
        super(context);
        init();
    }

    public CropViewEEFY(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    public void cropImage() {
        if (bitmap != null && cropRect.width() > 0 && cropRect.height() > 0) {
            ...
            if (getContext() instanceof MiddleActivityEEFY) {
                ((MiddleActivityEEFY) getContext()).setCroppedImage(croppedBitmap); // Pass cropped bitmap to MainActivity
            }
        }
    }
}
