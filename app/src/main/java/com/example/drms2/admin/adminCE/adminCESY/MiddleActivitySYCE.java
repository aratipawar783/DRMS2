...
public class MiddleActivitySYCE extends AppCompatActivity {

    private CropViewSYCE cropView;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_middle_syce);
        ...
        sharedPreferences = getSharedPreferences("ScreenshotAppsyce", MODE_PRIVATE);

        // Check if there's an image passed from the dashboard
        byte[] byteArray = getIntent().getByteArrayExtra("screenshotsyce");
        if (byteArray != null) {
        ...
        sendButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (croppedImage != null) {
                    ...
                    Toast.makeText(MiddleActivitySYCE.this, "Image saved. You can navigate back to the dashboard.", Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(MiddleActivitySYCE.this, "Please crop the image first.", Toast.LENGTH_SHORT).show();
                }
            }
        });
    ...
    private void saveImageToFile(Bitmap bitmap) {
        File file = new File(getExternalFilesDir(null), "cropped_image_middlesyce.png");
        try (FileOutputStream out = new FileOutputStream(file)) {
            bitmap.compress(Bitmap.CompressFormat.PNG, 100, out);
            out.flush();
            // Save the image URI to SharedPreferences
            SharedPreferences.Editor editor = sharedPreferences.edit();
            editor.putString("imageUrisyceForMiddle", file.getAbsolutePath());
            editor.apply();
        } catch (IOException e) {
            e.printStackTrace();
