...
public class CropViewMEFY extends View {
    ...
    public CropViewMEFY(Context context) {
        super(context);
        init();
    }

    public CropViewMEFY(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    public void cropImage() {
        if (bitmap != null && cropRect.width() > 0 && cropRect.height() > 0) {
            ...
            if (getContext() instanceof MiddleActivityMEFY) {
                ((MiddleActivityMEFY) getContext()).setCroppedImage(croppedBitmap); // Pass cropped bitmap to MainActivity
            }
        }
    }
}
