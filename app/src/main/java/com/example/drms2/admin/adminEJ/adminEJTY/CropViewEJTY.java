...
public class CropViewEJTY extends View {
    ...
    public CropViewEJTY(Context context) {
        super(context);
        init();
    }

    public CropViewEJTY(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    public void cropImage() {
        if (bitmap != null && cropRect.width() > 0 && cropRect.height() > 0) {
            ...
            if (getContext() instanceof MiddleActivityEJTY) {
                ((MiddleActivityEJTY) getContext()).setCroppedImage(croppedBitmap); // Pass cropped bitmap to MainActivity
            }
        }
    }
}
