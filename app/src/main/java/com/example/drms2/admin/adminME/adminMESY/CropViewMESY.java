...
public class CropViewMESY extends View {
    ...
    public CropViewMESY(Context context) {
        super(context);
        init();
    }

    public CropViewMESY(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    public void cropImage() {
        if (bitmap != null && cropRect.width() > 0 && cropRect.height() > 0) {
            ...
            if (getContext() instanceof MiddleActivityMESY) {
                ((MiddleActivityMESY) getContext()).setCroppedImage(croppedBitmap); // Pass cropped bitmap to MainActivity
            }
        }
    }
}
