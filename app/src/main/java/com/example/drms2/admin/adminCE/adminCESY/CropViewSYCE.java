...
public class CropViewSYCE extends View {
    ...
    public CropViewSYCE(Context context) {
        super(context);
        init();
    }

    public CropViewSYCE(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    public void cropImage() {
        if (bitmap != null && cropRect.width() > 0 && cropRect.height() > 0) {
            ...
            if (getContext() instanceof MiddleActivitySYCE) {
                ((MiddleActivitySYCE) getContext()).setCroppedImage(croppedBitmap); // Pass cropped bitmap to MainActivity
            }
        }
    }
}
