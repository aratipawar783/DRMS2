...
public class CropViewMETY extends View {
    ...
    public CropViewMETY(Context context) {
        super(context);
        init();
    }

    public CropViewMETY(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    public void cropImage() {
        if (bitmap != null && cropRect.width() > 0 && cropRect.height() > 0) {
            ...
            if (getContext() instanceof MiddleActivityMETY) {
                ((MiddleActivityMETY) getContext()).setCroppedImage(croppedBitmap); // Pass cropped bitmap to MainActivity
            }
        }
    }
}
