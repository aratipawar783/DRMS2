...
public class CropViewFYAE extends View {
    ...
    public CropViewFYAE(Context context) {
        super(context);
        init();
    }

    public CropViewFYAE(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    public void cropImage() {
        if (bitmap != null && cropRect.width() > 0 && cropRect.height() > 0) {
            ...
            if (getContext() instanceof MiddleActvivityFYAE) {
                ((MiddleActvivityFYAE) getContext()).setCroppedImage(croppedBitmap); // Pass cropped bitmap to MainActivity
            }
        }
    }
}
