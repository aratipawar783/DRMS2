...
public class CropViewEESY extends View {
    ...
    public CropViewEESY(Context context) {
        super(context);
        init();
    }

    public CropViewEESY(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    public void cropImage() {
        if (bitmap != null && cropRect.width() > 0 && cropRect.height() > 0) {
            ...
            if (getContext() instanceof MiddleActivityEESY) {
                ((MiddleActivityEESY) getContext()).setCroppedImage(croppedBitmap); // Pass cropped bitmap to MainActivity
            }
        }
    }
}
