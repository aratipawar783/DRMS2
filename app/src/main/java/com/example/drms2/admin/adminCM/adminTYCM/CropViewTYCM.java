...
public class CropViewTYCM extends View {
    ...
    public CropViewTYCM(Context context) {
        super(context);
        init();
    }

    public CropViewTYCM(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    public void cropImage() {
        if (bitmap != null && cropRect.width() > 0 && cropRect.height() > 0) {
            ...
            if (getContext() instanceof MiddleActivityTYCM) {
                ((MiddleActivityTYCM) getContext()).setCroppedImage(croppedBitmap); // Pass cropped bitmap to MainActivity
            }
        }
    }
}
