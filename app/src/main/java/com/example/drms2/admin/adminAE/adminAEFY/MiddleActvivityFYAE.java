package com.example.drms2.admin.adminAE.adminAEFY;
...
public class MiddleActvivityFYAE extends AppCompatActivity {

    private CropViewFYAE cropView;
    private Bitmap screenshotaefy; // Store the original screenshot
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_middle_actvity_fyae);
        ...
        sharedPreferences = getSharedPreferences("ScreenshotAppfyae", MODE_PRIVATE);

        // Check if there's an image passed from the dashboard
        byte[] byteArray = getIntent().getByteArrayExtra("screenshotfyae");
        if (byteArray != null) {
            screenshotaefy = BitmapFactory.decodeByteArray(byteArray, 0, byteArray.length);
            cropView.setBitmap(screenshotaefy);
        } else {
        ...
        sendButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (croppedImage != null) {
                    ...
                    Toast.makeText(MiddleActvivityFYAE.this, "Image saved. You can navigate back to the dashboard.", Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(MiddleActvivityFYAE.this, "Please crop the image first.", Toast.LENGTH_SHORT).show();
                }
            }
        });
    ...
    private void saveImageToFile(Bitmap bitmap) {
        File file = new File(getExternalFilesDir(null), "cropped_image_middlefyae.png");
        try (FileOutputStream out = new FileOutputStream(file)) {
            bitmap.compress(Bitmap.CompressFormat.PNG, 100, out);
            out.flush();
            // Save the image URI to SharedPreferences
            SharedPreferences.Editor editor = sharedPreferences.edit();
            editor.putString("imageUrifyaeForMiddle", file.getAbsolutePath());
            editor.apply();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private void clearCroppedImage() {
        ...
        cropView.setBitmap(screenshotaefy); // Reset the CropView with the original screenshot
